...
export const getAllUsers_event = () => async (dispatch) => {
  try {
    ...
    const { data } = await axios.get(`http://localhost:2000/api/eveent/user`);
    // console.log("res =>", data.users);
...
console.log('id',id)
    ...
    const { data } = await axios.put(`http://localhost:2000/api/editprofile/global/${id}`, userData, config);
...
export const addSponsor = (userData) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.post(`http://localhost:2000/api/add/global`, userData, config);
